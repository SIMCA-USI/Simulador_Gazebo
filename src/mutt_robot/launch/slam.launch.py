...
def generate_launch_description():
    mutt_robot_dir = get_package_share_directory("mutt_robot")
    nav2_bringup_dir = get_package_share_directory("nav2_bringup")
    slam_toolbox_dir = get_package_share_directory("slam_toolbox")
    
    # Params files
    nav2_params_file = os.path.join(mutt_robot_dir, "config", "nav2_slam_params.yaml")
    rl_params_file = os.path.join(mutt_robot_dir, "config", "dual_ekf_navsat_params.yaml")
    
    # 1. Gazebo + Robot Spawn (Reuse existing launch)
    gazebo_model_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(mutt_robot_dir, "launch", "gazebo_model.launch.py")
        )
    )
    ...
    slam_params_file = os.path.join(mutt_robot_dir, "config", "mapper_params_online_async.yaml")
